...
**v2ray-haproxy-tor-obfs4** — это решение для маршрутизации трафика через сеть Tor с использованием нескольких параллельных экземпляров Tor, каждый из которых работает через обфусцированные мосты (obfs4). Проект реализован в виде набора Docker-контейнеров, управляемых через Docker Compose. Предназначен для обеспечения приватного выхода в сеть Tor как из локальной сети, так и из внешней сети, с балансировкой трафика между экземплярами Tor.
