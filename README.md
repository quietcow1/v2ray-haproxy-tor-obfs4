...
**v2ray-haproxy-tor-obfs4** — это решение для маршрутизации трафика через сеть Tor с использованием нескольких параллельных экземпляров Tor, каждый из которых работает через обфусцированные мосты (obfs4). Проект реализован в виде набора Docker-контейнеров, управляемых через Docker Compose. Предназначен для обеспечения приватного выхода в сеть Tor как из локальной сети (через Privoxy), так и из внешней сети (через V2Ray), с балансировкой трафика между экземплярами Tor.
...
- Подключение возможно:
  ...
  - из **внешней сети** через **V2Ray** (VMess + WebSocket + TLS, с проксированием через Caddy)
  - из **локальной сети** через **Privoxy** (HTTP) или **V2Ray**
...
1. **Из внешней сети** — через V2Ray (VMess + WebSocket + TLS) с проксированием через Caddy.
2. **Из локальной сети** — через Privoxy (HTTP).
...
Она не имеет русской локализации "из коробки", но предлагает расширенные функции.
